...
output_path = os.path.join(cfg.FORECASTS_DIR, "svr")
...
for country in cfg.COUNTRIES:
    ...
    best_rmse = float("inf")
    best_params = None
    for c_val in cfg.SVR_PARAMS["C"]:
        for epsilon in cfg.SVR_PARAMS["epsilon"]:
            estimator = Pipeline([("svr", SVR(C=c_val, epsilon=epsilon))])
            rmse = rolling_origin_rmse(estimator, X_train, y_train, min_train_size=5)
            if rmse < best_rmse:
                best_rmse = rmse
                best_params = (c_val, epsilon)

    c_val, epsilon = best_params
    final_model = SVR(C=c_val, epsilon=epsilon)
    ...
    print(
        f"{country} | best C={c_val}, epsilon={epsilon}, CV-RMSE={best_rmse:.3f} "
        f"-> Train RMSE: {train_rmse:.3f}, MAE: {train_mae:.3f}, MAPE: {train_mape:.2f}% "
        f"| Test RMSE: {rmse:.3f}, MAE: {mae:.3f}, MAPE: {mape_val:.2f}%"
    )

    forecast_df.to_csv(os.path.join(output_path, f"{country}_svr_best.csv"))
